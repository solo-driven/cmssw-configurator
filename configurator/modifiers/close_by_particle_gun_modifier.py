...
def generator_string(attributes_dict):
    params = attributes_dict['parameters']
    repr = f"""
    process.generator = cms.EDProducer("CloseByParticleGunProducer",
        AddAntiParticle = cms.bool({attributes_dict['add_anti_particle']}),
        ...
        Verbosity = cms.untracked.int32({attributes_dict['verbosity']}),
        firstRun = cms.untracked.uint32({attributes_dict['first_run']}),
        psethack = cms.string("{attributes_dict['psethack']}")
    )
    """
    import textwrap
